...
for(let i=0;i < counter ; i++){
  ...
  console.log(deserializedMacaroonAlice.inspect())
  ...
  const {dischargeMacaroon: dischargeMacaroonJane} = await mbacsaClient.dischargeDelegationToken(POD_SERVER_1_BASE_PATH + ENDPOINT_DISCHARGE, 
    {serializedRootMacaroon: attenuatedMacaroonJane.serialize(), agentToDischarge: "http://localhost:3000/Jane/profile/card#me"}, dpopDataJane );
  ...
  // // Jane tries to access post but should be denied since Alice revoked her access
  // const revokedPostData = await mbacsaClient.accessWithDelegationToken("http://localhost:3000/Bob/social/post1.txt",[attenuatedMacaroonJane.serialize(),dischargeMacaroonAlice,dischargeMacaroonJane])
  // console.log(revokedPostData)
